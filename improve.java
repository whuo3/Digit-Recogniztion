...
public class improve {
    ...
    private static int getValIndex(char[][] digit, int x, int y){
        ...
        for(int k = y - 1; k >= 0; k--) if(digit[x][k] != ' ') {leftStar = true;break;}
        for(int k = y + 1; k < digit[0].length; k++) if(digit[x][k] != ' ') {rightStar = true;break;}
        if(leftStar&&rightStar) index = index*4;
        else if(leftStar) index = index*4+1;
        else if(rightStar) index = index*4+2;
        else index = index*4+3;
    ...
    private static boolean readDigitTestImage() throws IOException{
        ...
        if(labelY == 5 && predictLabel !=5) five.add(predictLabel);
    ...
    /********** evaluation ********/
    static ArrayList<Integer> five = new ArrayList<Integer>();
    ...
    public static void main (String [] args) throws Exception {
        ...
        trainData();
        testData();
        if(disjoint) System.out.println("improved and disjoint");
        else System.out.println("improved and Overlap");
